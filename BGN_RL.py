...
class BGN_RL:
    def __init__(self, pd, tmax=1000, n=10, seed=None):
        np.random.seed(seed)
        self.pd = pd
        self.tmax = tmax
        self.dt = 0.01
        ...
        self.Cm = 1
        self.action_space = [(0, 0)] + [(i, j) for i in range(100, 500, 50) for j in range(50, 180, 20)]
        ...
        self.EI = 0
        self.eis = np.zeros(len(self.t))

        self.i = 2
        self.reward = -100
        self.state = None
        self.sgi = np.zeros((10, self.tmax*100-1))


    def step(self, action=None, step_size=100):
        ...
        for mini_step in range(step_size):  
            ...
            self.Iahp2 = self.gahp[1] * (self.V2 - self.Ek[1]) * (self.CA2 / (self.CA2 + self.k1[1])) 
            self.Igesn = 0.5 * (self.gsyn[0] * (self.V2 - self.Esyn[0]) * (self.S3 + self.S31))  
            ...
            self.Iahp3 = self.gahp[2] * (self.V3 - self.Ek[2]) * (self.CA3 / (self.CA3 + self.k1[2])) 
            self.Isnge = 0.5 * (self.gsyn[1] * (self.V3 - self.Esyn[1]) * (self.S2 + self.S21))  
            self.Igege = 0.5 * (self.gsyn[2] * (self.V3 - self.Esyn[2]) * (self.S31 + self.S32))  
            ...
            self.Iahp4 = self.gahp[2] * (self.V4 - self.Ek[2]) * (self.CA4 / (self.CA4 + self.k1[2])) 
            self.Isngi = 0.5 * (self.gsyn[3] * (self.V4 - self.Esyn[3]) * (self.S2 + self.S21))  
            self.Igigi = 0.5 * (self.gsyn[4] * (self.V4 - self.Esyn[4]) * (self.S31 + self.S32)) 
            self.Iappgpi = 22 - self.pd * 6  

            self.vth[:, i-1] = self.V1 + self.dt * (1 / self.Cm * (-self.Il1 - self.Ik1 - self.Ina1 - self.It1 - self.Igith + self.Istim[i-1]))
            self.H1 = self.H1 + self.dt * ((self.h1 - self.H1) / self.th1)
            self.R1 = self.R1 + self.dt * ((self.r1 - self.R1) / self.tr1)
            current = 0
       

            self.vsn[:, i-1] = self.V2 + self.dt * (1 / self.Cm * (-self.Il2 - self.Ik2 - self.Ina2 - self.It2 - self.Ica2 - self.Iahp2 - self.Igesn + self.Iappstn + current))
            ...
            self.CA2 = self.CA2 + self.dt * (3.75 * 10**-5 * (-self.Ica2 - self.It2 - self.kca[1] * self.CA2))
            ...
            self.vge[:, i-1] = self.V3 + self.dt * (1 / self.Cm * (-self.Il3 - self.Ik3 - self.Ina3 - self.It3 - self.Ica3 - self.Iahp3 - self.Isnge - self.Igege + self.Iappgpe))
            ...
            self.CA3 = self.CA3 + self.dt * (1 * 10**-4 * (-self.Ica3 - self.It3 - self.kca[2] * self.CA3))
            self.S3 = self.S3 + self.dt * (self.A[2] * (1 - self.S3) * Hinf(self.V3 - self.the[2]) - self.B[2] * self.S3)
            # self.aGPe[:,i-1]=(self.vge[:,i-2]<-10 & self.vge[:,i-1]>-10)

            self.vgi[:, i-1] = self.V4 + self.dt * (1 / self.Cm * (-self.Il4 - self.Ik4 - self.Ina4 - self.It4 - self.Ica4 - self.Iahp4 - self.Isngi - self.Igigi + self.Iappgpi))
            ...
            self.CA4 = self.CA4 + self.dt * (1 * 10**-4 * (-self.Ica4 - self.It4 - self.kca[2] * self.CA4))
            ...
            if i-50 in self.smc_pulse*100:
                smc_pulse_index = np.where(self.smc_pulse*100==i-50)[0][0]
                self.EI = self.calculate_ei(smc_pulse_index)
            

            self.i = self.i+1

            if i == self.tmax*100:
                self.state = np.array(self.EI, self.PB)
    ...
    def calculate_bursts(self, vgi_reading):
        ...
        for i in range(self.n):
            arr = vgi_reading[i]
            peaks = peaks = find_peaks(arr, height=40)[0]
            for j in range(len(peaks)-1):
                dists.append(np.abs(peaks[j] - peaks[j+1]))
        bursts = len(np.where(np.array(dists) < 1000)[0])/self.n
        return bursts
    
    def calculate_pb(self, start, end):
        sum = 0
        for i in range(self.n):
            sum += sum(abs(np.fft.fft(self.vgi[i][start:end:25]))[20:35])

        return sum
    
    def calculate_ei(self, b2=None):
        ...
        b1 = 0
        if not b2: b2 = len(self.smc_pulse)
        ...
        for i in range(m):  # Loop over each thalamic cell
            ...
            for p in range(b1, b2):  # Loop over each self.smc_pulse
                if p != b2 - 1:
                    ...
                    a = [c for c in compare if self.smc_pulse[p] <= c < self.smc_pulse[p] + 25]
                    # Next spike window
                    b = [c for c in compare if self.smc_pulse[p] + 25 <= c < self.smc_pulse[p + 1]]
                else:
