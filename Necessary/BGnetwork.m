%function [EI,aGPe1] = BGnetwork(pd,wstim,freq)
%changed to
function [EI,Pb] = BGnetwork(pd,wstim,freq)
...
%%additional trackers
% h2t = zeros(n,length(t));
% n2t = zeros(n,length(t));
% r2t = zeros(n,length(t));
% c2t = zeros(n,length(t));
% ca2t = zeros(n,length(t));
% ISave = zeros(n,length(t));
n3t = zeros(n,length(t));
h3t = zeros(n,length(t));
r3t = zeros(n,length(t));
ca3t = zeros(n,length(t));
s2t = zeros(n,length(t));
s3t = zeros(n,length(t));
ISaveSTN = zeros(n,length(t));
ISaveGPE = zeros(n,length(t));
%%end additional trackers
...
%%Time loop
aGPe = zeros(n,length(t));
aSTN1 = zeros(n,length(t));
for i=2:length(t)        
...
fftResult = abs(fft(vgi(1,2:25:end), 2048*2));
% figure(1); plot(fftResult')
%Pb = fftResult(13); original code
Pb = sum(fftResult(13:30));
%disp(EI)
%disp(Pb);
...
aGPe1(1:n-1,:)=aGPe(2:n,:);aGPe1(n,:)=aGPe(1,:); %connections from gpe/stn shuffled
aGPe2(3:n,:) = aGPe(1:n-2,:); aGPe2(1:2,:) = aGPe(n-1:n,:);
S21(2:n)=S2(1:n-1);S21(1)=S2(n);
aSTN2(2:n,:) = aSTN1(1:n-1,:) ; aSTN2(1,:) = aSTN1(n,:);
%figure
%plot(ISave(1,:));
save('LatestGPe.mat','n3t','Idbs', 'h3t','r3t','ca3t','s2t','s3t','t','vge','aGPe1','aGPe2','aSTN1','aSTN2','Istim','timespike','tmax','dt','v1','v2','v3','v4','r','n','vgi','vth', 'vsn');
