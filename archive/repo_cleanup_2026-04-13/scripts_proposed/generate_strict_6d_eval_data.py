"""
Generate strict 6D calibration/held-out splits and stochastic replay episodes.

The cached 6D environment in this repository is derived from one precomputed MATLAB
signal bundle. For a paper-ready quantization evaluation, we need:

1. A calibration split used only for INT8 representative-data generation.
2. A held-out split that is disjoint from calibration windows.
3. A replay dataset of stochastic held-out trajectories so we can compare FP32 vs
   INT8 controller behavior on varied episodes instead of a deterministic smoke test.
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Generate strict 6D evaluation data splits.")
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="results/strict_eval",
        help="Directory to store strict calibration/held-out/replay artifacts.",
    )
    ...
    if not getattr(env, "has_precomputed_data", False):
        raise SystemExit("Cached strict 6D split generation requires precomputed MATLAB data.")
