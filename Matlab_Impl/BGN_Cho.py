...
class BGN_Cho(gym.Env):
    ...
    def step(self, action=np.array([-1.0, -1.0]), sim_time=10000):
        ...
        r2_max = 1224.744871391589
        self.dbs = scipy.io.loadmat('C:/Users/ncart/Programming/DBS/Matlab_Impl/bgn_vars.mat')['dbs'].flatten()
        sum = 0
        for j in range(sim_time): sum += int(self.dbs[j]**2)
        r2 = np.sqrt(1/sim_time * sum)/r2_max
